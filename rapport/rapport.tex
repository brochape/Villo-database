...
\begin{itemize}
\item Date mise en service d'un vélo < date de départ d'un trajet avec ce vélo < date d'arrivée de ce trajet
\item Date d'expiration du compte utilisateur > date d'inscription de ce compte
\item Date d'expiration du compte utilisateur > heure de départ d'un nouveau trajet de cet utilisateur
...
\item Le nombre de vélos dans une station donnée <= capacité de cette station
\item Deux stations différentes ne peuvent pas avoir les mêmes coordonnées GPS
\item Le nombre de vélos dans une station donnée >= 0
\item Un utilisateur ne peut pas se connecter après la date d'expiration
\item Le nombre de vélos à la station de départ d'un trajet à l'heure du départ > 0
\item Le nombre de vélos à la station d'arrivée à l'heure d'arrivée < capacité de cette station
\end{itemize}
\pagebreak
...
\item Pour tout Trajet, Trajet.HeureDépart doit etre plus grand que Vélo.DateMiseEnService avec Trajet.NumVeo == Vélo.NumVelo
...
Pour mieux refléter ces changements, le programme d'importation des données effectue deux tâches importantes.
Tout d'abord, il crée des \textbf{paymentDate} pour chaque \textbf{tempUsers} afin que la base de données soit cohérente. 
